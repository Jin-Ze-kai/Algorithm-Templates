...
```bash
g++ -std=c++20 -O2 -Wall a.cpp -o a && ./a
```

如果有while (cin>>n)需要Ctrl+D发送EOF信号

如果死循环就Ctrl+C

### Dev-C++

C++11不支持结构化绑定，不支持gcd，auto函数需要->type，不支持Lambda递归，替代方法：

```c++
function<void(int,int)> dfs=[&](int u,int fa) {};
```

使用代码补全需要万能头，地址：Dev-Cpp\MinGW64\lib\gcc\x86_64-w64-mingw32\4.9.2\include\c++\x86_64-w64-mingw32\bits

快捷键选项：改代码补全的快捷键

编译选项：编译器改成64-bit Debug；代码生成优化级别改成Debug；语言标准改成GNU C++11
...
//auto it0=prev(it);
//auto it1=next(it);
vector<decltype(st)::iterator> vec;
for(auto it=st.begin();it!=st.end();) {
	if ((*it)[0]%2) it=st.erase(it);
    else vec.push_back(it),++it;
}
```

### extc++.h平衡树

支持set的操作+O(logN)的排名查询
...
如果除了源点的每个顶点只能流过1单位流量，复杂度为 **$\mathcal{O}(E\sqrt{V})$** 。

一般网络的复杂度为 **$\mathcal{O}(V^2 E)$**。

如果在二分图上动态单次加入 $m$ 条边，因为遍历一遍Residual Graph的开销是 $\mathcal{O}(E)$ ，所以总的是 $$\mathcal{O}(m \cdot E)$$。
...
查找最小生成树
 ...
 $$\mathcal{O}(n^2)$$，稠密图/完全图（$m=n^2$）时使用
...
高维前缀和，dp[mask]表示所有mask子集对应的权值之和
...
r数组存的是扩展串半径，扩展串半径/2就是回文子串数量

真实长度=扩展串半径-1

扩展串中心=原串起始+原串终止+1

原串终止位置=扩展串结尾下标/2=(i+r[i]-1)/2
